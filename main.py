    ...
    for i in bannedwords:
        if i in message.content.lower():
            ...
            if bannedwords[i] == 'Racial Abuse':
                ...
                await message.channel.send(msg)
                dembed = discord.Embed(title=f"**Infraction information**",
                                       description=f"\n**NAME: **{message.author.name}\n**TYPE: **Ban\n**REASON: **Usage of Racial Abuse inside the community.\n**ATTEMPT: **1\n**DURATION: **Permenent",
                                       color=0xFF0000)
                dembed.set_footer(text="Tortoise Community")
                dmembed = discord.Embed(title=f"**Infraction information**",
                                        description=f"""\n**TYPE: **Ban\n**REASON: **Usage of Racial Abuse inside the community.  \n**DURATION: **Permenent\n""",
                                        color=0xFF0000)
            ...
            if staff in message.author.roles:
                ...
                if bannedwords[i] == 'Discord link':
                    ...
                    if Advertiser_id in Warned_once:
                        spembed = discord.Embed(title=f"**Infraction information**",
                        ...
                        spdembed = discord.Embed(title=f"**Infraction information**",
                                                 description=f"\n**TYPE: **Ban\n**REASON: **Self Promotion \n**ATTEMPT: **2\n**DURATION: **Permenent",
                                                 color=0xFF0000)
                        ...
                        dembed = discord.Embed(title="**Warning!**",
                                               description=f"Discord Link detected by **{message.author.mention}** in {message.channel.mention}\n  {message.author.name} Self Promotion is **Forbidden** inside the Community \n**ATTEMPTS:** 1",
                                               color=0xFF8B00)
            ...
            if bannedwords[i] == 'Racial Slur' or bannedwords[i] == 'Homophobic Slur':
                ...
                if attempt == '2':
                    dembed = discord.Embed(title="Warning!",
                                           description=f"We have detected your use of {reason} which are **STRICTLY PROHIBITED** inside the community.\nThis an infringement of our server rules stated in <#591662973307584513>.\nConsider this as the **FINAL WARNING**. If you are picked up repeating this again, Direct action will be taken.\nIf this happened by a mistake please contact one of the moderators to review the messages",
                                           color=0xFF8B00)
                    dembed.set_author(name="Tortoise Community", icon_url="https://i.ibb.co/rxM1zqC/bot-2.png")
...
async def on_guild_join(ctx):
    ...
    embed = discord.Embed(
        description="I'm the Tortoise BOT (beta. I'm here for Bot testing and development.  use `T.` before a command",
        color=0x3498db)
...
class admins(commands.Cog):
    ...
    async def warn(self, ctx, member: discord.Member, *, reason):
        ...
        embed = discord.Embed(title=f"**{member.name} You have been warned for {reason}**",
                              description=f" If you are planning to repeat this again, the mods may administer punishment for the action.",
                              color=0xF4D03F)
    ...
    async def role(self, ctx, role_: discord.Role, member: discord.Member):
        ...
        if member is None:
            ...
            embed = discord.Embed(title=f"Role Added!", description=f"{member.mention} is now a {role_.name}",
                                  color=0x1ADB43)
            dmbed = discord.Embed(
...
class other(commands.Cog):
    ...
    async def github(self, ctx):
        ...
        embed = discord.Embed(title="""Tortoise-BOT github repository""",
                              url="https://github.com/Tortoise-Community/Tortoise-BOT", color=0x206694)
...
class tortoise_server(commands.Cog):
    ...
    async def accept(self, ctx):
        if ctx.message.channel.id == 602156675863937024:
            if len(ctx.message.author.roles) == 2:
                ...
                syslog = discord.Embed(
                    title=f"""**Welcome!**\n{ctx.message.author.name} has joined the Tortoise Community""",
                    color=0x13D910)
    ...
    async def events(self, ctx, arg):
        ...
        embed = discord.Embed(title="**Success!**", description=f"Events Turned {arg.upper()} successfully  ",
                              color=0x13D910)
        await ctx.send(embed=embed)
    ...
    async def announcements(self, ctx, arg):
        ...
        embed = discord.Embed(title="**Success!**",
                              description=f"Announcements Turned **{arg.upper()}** successfully  ", color=0x13D910)
        await ctx.send(embed=embed)
        ...
        if ctx.channel.id == 580809054067097600:
            ...
            for i in data["reporters"]:
                if i["status"] == "false":
                    ...
                    gembed = discord.Embed(title=f"You can chat and resolve the issue here:",
                                           description=f"**MEMBER ONLINE:** {member.name} \n type `t.stop` to end the session. ",
                                           color=0xF2771A)
    ...
    async def submit(self, ctx):
        ...
        if Event == "off":
            ...
            if contestant in participated:
                embed = discord.Embed(
                    title=f"""It seems you have already participated in the contest.\nYou cannot submit the code again.""",
                    color=0xFF0000)
                await ctx.send(embed=embed)
            else:
                ...
                try:
                    ...
                    submitted.write("\n" + contestant)
                    embed = discord.Embed(title=f"""{ctx.author.name} has submitted the code!:thumbsup:""",
                                          color=0xF2771A)
